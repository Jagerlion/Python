...
def iso_8601(s):
    ...
    assert len(s) == 8, 'The string '+repr(s)+' is not 8 characters long'
    assert introcs.count_str(s,':') == 2, 'The string '+repr(s)+' does not have two colons.'
...
def str_to_seconds(time):
    ...
    assert type(time) == str, 'The value '+repr(time)+' is not a string.'
    assert len(time) == 8, 'The string '+repr(time)+' is not 8 characters long'
    assert iso_8601(time) == True, 'The string '+repr(time)+' is not in ISO format'

    total_seconds = (get_hours(time)*60*60) + (get_minutes(time)*60) + get_seconds(time)
